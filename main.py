...
def make_tale(frame_pa, music, row_x, la, bg='#121212', fg="#f3f3f3", leave_colo='#2a2a2a'):
     # Function to handle mouse entering event
    def on_enter(event, music_info, index_framee, index, music_player_subtitle, music_title, music_artist, music_time, music_add):
    ...
    def on_leave(event, music_info, index_framee, index, music_player_subtitle, music_title, music_artist, music_time, music_add):
    ...
    if la:
        ...
        music_option.bind("<Button-1>", lambda event, music=music: show_listbox(event, music))
    else:
        music_option = Label(music_info, text=' 『+』 ', fg=fg, bg=bg, height=1)
        music_option.config(font=("", 20))
        music_option.grid(row=0, column=3)
        music_option.bind("<Button-1>", lambda event, current_song_info=music:add_song(event, current_song_info))
        return
    ...
    music_info.bind("<Enter>", lambda event, music_info=music_info, index_framee=index_framee, index=index,
                                    ...
                                    music_artist=music_artist, music_time=music_time, music_option=music_option:
                        on_enter(event, music_info, index_framee, index, music_player_subtitle, music_title,
                                music_artist, music_time, music_option))
...
def open_window():
    ...
    if window and window.winfo_exists():
        ...
        frame_window.pack(fill=BOTH, expand=True)

        canvas_window = Canvas(frame_window)
        canvas_window.pack(side=LEFT, fill=BOTH, expand=True)

        scrollbar = Scrollbar(frame_window, orient=VERTICAL, command=canvas_window.yview)
        scrollbar.pack(side=RIGHT, fill=Y)
        canvas_window.configure(yscrollcommand=scrollbar.set)

        interior_window = Frame(canvas_window)
        canvas_window.create_window((0, 0), window=interior_window, anchor=NW)

        def configure_interior(event):
            canvas_window.configure(scrollregion=canvas_window.bbox("all"))

        def on_canvas_configure(event):
            canvas_window.itemconfig(window_interior_id, width=event.width)
        
        canvas_window.bind("<Configure>", on_canvas_configure)
        interior_window.bind("<Configure>", configure_interior)
        cre_listsong(listsong, interior_window, False)
...
button = Button(controls_frame, text="List Song", command=open_window)
